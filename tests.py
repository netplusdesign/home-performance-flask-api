...
class ChartingPerformanceTestCase(unittest.TestCase):
    ...
    def test_defaults_years(self):
        ...
        assert 'Interval \'years\' does not exist' in rv.data.decode('utf-8')
    ...
    def x_test_views_summary_days(self): # errors not propagating, long term fix
        ...
        json_rv = json.loads(rv.data.decode('utf-8'))
        assert 'Interval \'days\' does not exist' in rv.data.decode('utf-8')
    ...
    def test_views_summary_days(self):
        ...
        json_rv = json.loads(rv.data.decode('utf-8'))
        assert json_rv['interval'] == 'day'
        assert len(json_rv['items']) == 31
        assert float(json_rv['items'][0]['net']) == 22.0340000000
        assert float(json_rv['totals']['net']) == 630.9560000000
    ...
    def test_views_summary_months(self):
        ...
        assert len(json_rv['items']) == 12
        assert float(json_rv['totals']['net']) == -3255.785
    ...
    def test_views_summary_years(self):
        ...
        json_rv = json.loads(rv.data.decode('utf-8'))
        assert json_rv['interval'] == 'year'
        assert len(json_rv['items']) == 1
        assert float(json_rv['totals']['net']) == -3255.785
    ...
    def test_views_summary_years_no_start_or_end(self):
        ...
        json_rv = json.loads(rv.data.decode('utf-8'))
        assert json_rv['interval'] == 'year'
        assert len(json_rv['items']) == 5
        assert float(json_rv['totals']['net']) == -8977.4961749210000

    def test_views_summary_months_no_duration(self):
        rv = self.app.get('/api/houses/0/views/summary/?interval=months&start=2016-01-01')
        json_rv = json.loads(rv.data.decode('utf-8'))
        assert json_rv['interval'] == 'month'
        assert len(json_rv['items']) == 9
        assert float(json_rv['totals']['net']) == -2560.7196077480000
    ...
    def test_views_generation_months(self):
        ...
        json_rv = json.loads(rv.data.decode('utf-8'))
        assert json_rv['interval'] == 'month'
        assert len(json_rv['items']) == 4
        assert float(json_rv['totals']['actual']) == -2465.356
    ...
    def test_views_generation_days(self):
        ...
        assert json_rv['interval'] == 'day'
        assert len(json_rv['items']) == 31
        assert float(json_rv['totals']['actual']) == -478.374
    ...
    def test_views_generation_hours(self):
        ...
        json_rv = json.loads(rv.data.decode('utf-8'))
        assert json_rv['interval'] == 'hour'
        assert len(json_rv['items']) == 72
        assert float(json_rv['totals']['actual']) == -14.660
    ...
    def test_views_hdd_months(self):
        ...
        assert json_rv['interval'] == 'month'
        assert len(json_rv['items']) == 4
        assert float(json_rv['totals']['actual']) == 3809.332
        assert float(json_rv['coldest_hour']['temperature']) == -7.089
        assert float(json_rv['coldest_day']['temperature']) == 60.770
    ...
    def test_views_temperature_years(self):
        ...
        assert json_rv['interval'] == 'year'
        assert len(json_rv['items']) == 5
        assert float(json_rv['totals']['avg_temperature']) == 50.1056564
        assert float(json_rv['totals']['max_temperature']) == 95.135
        assert float(json_rv['items'][0]['sum_hdd']) == 4759.847
        assert float(json_rv['items'][0]['avg_temperature']) == 53.5261340
    ...
    def test_views_water_months(self):
        ...
        assert json_rv['interval'] == 'month'
        assert len(json_rv['items']) == 12
        assert float(json_rv['totals']['cold']) == 16253.4
        assert float(json_rv['totals']['hot']) == 7868.9
        assert float(json_rv['totals']['main']) == 24122.3
        assert float(json_rv['totals']['water_heater']) == 2078.042
        assert float(json_rv['totals']['water_pump']) == 63.780
    ...
    def test_views_usage_months_summary_2014(self):
        ...
        for circuit in json_rv['circuits']:
            if circuit['circuit_id'] == 'all_other':
                assert float(circuit['actual']) == 1248.621306547
    ...
    def test_views_usage_months_all(self):
        ...
        assert len(json_rv['items']) == 12
        assert float(json_rv['totals']['actual']) == 7206.154
        assert float(json_rv['totals']['budget']) == 5950
        assert json_rv['items'][0]['date'] == '2013-01-01'
        assert float(json_rv['items'][0]['actual']) == 880.949
        assert float(json_rv['items'][0]['budget']) == 800
    ...
    def test_views_usage_days_all(self):
        ...
        assert json_rv['view'] == 'usage.all'
        assert len(json_rv['items']) == 31
        assert float(json_rv['totals']['actual']) == 880.949
        assert json_rv['items'][0]['date'] == '2013-01-01'
    ...
    def test_views_usage_hours_all(self):
        ...
        assert json_rv['view'] == 'usage.all'
        assert len(json_rv['items']) == 72
        assert float(json_rv['totals']['actual']) == 45.068
        assert json_rv['items'][0]['date'] == '2013-05-01T00:00:00'
    ...
    def test_views_usage_months_ashp(self):
        ...
        assert len(json_rv['items']) == 12
        assert float(json_rv['totals']['actual']) == 1195.782
        assert round(float(json_rv['totals']['hdd']), 4) == 3366.5438
        assert json_rv['items'][0]['date'] == '2013-01-01'
        assert float(json_rv['items'][0]['actual']) == 282.305
        assert round(float(json_rv['items'][0]['hdd']),5) == 730.24137
    ...
    def test_views_usage_days_ashp(self):
        ...
        assert json_rv['view'] == 'usage.ashp'
        assert len(json_rv['items']) == 31
        assert float(json_rv['totals']['actual']) == 282.305
        assert round(float(json_rv['totals']['hdd']), 4) == 730.2414
        assert json_rv['items'][0]['date'] == '2013-01-01'
        assert float(json_rv['items'][0]['actual']) == 7.875
    ...
    def test_views_usage_hours_ashp(self):
        ...
        assert json_rv['view'] == 'usage.ashp'
        assert len(json_rv['items']) == 72
        assert float(json_rv['totals']['actual']) == 38.290
        assert round(float(json_rv['totals']['hdd']), 4) == 106.6046
        assert json_rv['items'][0]['date'] == '2013-01-01T00:00:00'
        assert float(json_rv['items'][0]['actual']) == 0
    ...
    def test_views_usage_months_all_other(self):
        ...
        assert len(json_rv['items']) == 12
        assert float(json_rv['totals']['actual']) == 3098.636
        assert json_rv['items'][0]['date'] == '2013-01-01'
        assert float(json_rv['items'][0]['actual']) == 240.958
    ...
    def test_views_usage_months_all_other_with_new_circuits_2014(self):
        ...
        json_rv = json.loads(rv.data.decode('utf-8'))
        assert len(json_rv['items']) == 12
        assert float(json_rv['totals']['actual']) == 1248.621306547
        assert json_rv['items'][4]['date'] == '2014-05-01'
        assert float(json_rv['items'][4]['actual']) == 49.741706954
    ...
    def test_views_usage_months_water_heater(self):
        ...
        assert len(json_rv['items']) == 12
        assert float(json_rv['totals']['actual']) == 2078.042
        assert json_rv['items'][0]['date'] == '2013-01-01'
        assert float(json_rv['items'][0]['actual']) == 255.815
    ...
    def test_views_usage_days_water_heater(self):
        ...
        assert json_rv['view'] == 'usage.water_heater'
        assert len(json_rv['items']) == 31
        assert float(json_rv['totals']['actual']) == 170.651
        assert json_rv['items'][0]['date'] == '2013-05-01'
    ...
    def test_views_usage_hours_water_heater(self):
        ...
        assert json_rv['view'] == 'usage.water_heater'
        assert len(json_rv['items']) == 72
        assert float(json_rv['totals']['actual']) == 25.848
        assert json_rv['items'][0]['date'] == '2013-01-01T00:00:00'
    ...
    def test_views_usage_months_water_pump(self):
        ...
        json_rv = json.loads(rv.data.decode('utf-8'))
        assert len(json_rv['items']) == 12
        assert float(json_rv['totals']['actual']) == 63.780
        assert json_rv['items'][0]['date'] == '2013-01-01'
        assert float(json_rv['items'][0]['actual']) == 5.916
    ...
    def test_views_usage_months_dryer(self):
        ...
        json_rv = json.loads(rv.data.decode('utf-8'))
        assert len(json_rv['items']) == 12
        assert float(json_rv['totals']['actual']) == 224.337
        assert json_rv['items'][0]['date'] == '2013-01-01'
        assert float(json_rv['items'][0]['actual']) == 33.748
    ...
    def test_views_usage_months_washer(self):
        ...
        json_rv = json.loads(rv.data.decode('utf-8'))
        assert len(json_rv['items']) == 12
        assert float(json_rv['totals']['actual']) == 44.702
        assert json_rv['items'][0]['date'] == '2013-01-01'
        assert float(json_rv['items'][0]['actual']) == 5.006
    ...
    def test_views_basetemp_months(self):
        ...
        assert round(float(json_rv['points'][0]['hdd']), 6) == 635.079083
        assert float(json_rv['points'][0]['ashp']) == 237.696
        assert float(json_rv['points'][0]['temperature']) == 23.3554699
        assert float(json_rv['points'][0]['solar']) == -10.008
    ...
    def test_views_chart(self):
        ...
        assert float(json_rv['hours'][0]['net']) == 224
        assert float(json_rv['hours'][0]['solar']) == 0
        assert float(json_rv['hours'][0]['first_floor_temp']) == 65.851
        assert float(json_rv['hours'][0]['hdd']) == 1.417
    ...
    def test_views_heatmap(self):
        ...
        assert float(json_rv['days'][0]['net']) == 26.881
        assert float(json_rv['days'][0]['solar']) == 0
        assert float(json_rv['days'][0]['outdoor_deg_min']) == 10.179
        assert float(json_rv['days'][0]['hdd']) == 40.752
